...
public class RecoveryDecisionService {
     ...
     * 可恢复的任务状态
     */
    private static final Set<TaskStatus> RECOVERABLE_STATUSES = new HashSet<>(Arrays.asList(
     ...
     * ALWAYS策略: 只要任务未完成就恢复
     */
    private boolean shouldAlwaysRecover(TaskExecution execution) {
        // 已成功的任务不恢复
        if (execution.getStatus() == TaskStatus.SUCCESS) {
            return false;
        }
     ...
     * AUTO策略: 智能判断
     */
    private boolean shouldAutoRecover(TaskExecution execution, RecoveryPolicy policy) {
        // 检查状态是否可恢复
     ...
     * 判断状态是否可恢复
     */
    public boolean isRecoverableStatus(TaskStatus status) {
     ...
     * 计算下次重试时间
     */
    public long calculateNextRetryTime(TaskExecution execution, RecoveryPolicy policy, long currentTimeMillis) {
